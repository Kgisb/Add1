...
if view == "Marketing Plan":
    ...
    def _month_planning_tab():
        ...
        with left:
            ...
            target_month = st.date_input("Target Month", value=default_month).replace(day=1)
        with right:
            plan_mode = st.radio("Planning Mode", ["Leads → Enrollments","Goal → Leads"], horizontal=True)

        # Look-back controls with +/- steering
        c1, c2, c3 = st.columns([1,1,2])
        with c1:
            lb_len = st.number_input("Look-back length (months)", min_value=1, max_value=60, value=3, step=1)
        with c2:
            ...
            anchor = st.date_input("Look-back anchor (ends)", value=anchor_default).replace(day=1)
        with c3:
        ...
        if _cty and _cty in df.columns:
            ...
            all_cty = []
        top_all = st.checkbox("All countries", value=True)
        top_n = 0
        if not top_all:
            top_n = st.number_input("Top N countries", min_value=1, max_value=max(1,len(all_cty)) if all_cty else 1, value=min(3, len(all_cty) or 1), step=1)
        ...
        if plan_mode == "Leads → Enrollments":
            ...
            edited = st.data_editor(inp, num_rows="dynamic", use_container_width=True, key="mp_leads_inp")
            # Compute outputs
            ...
            edited = st.data_editor(inp, num_rows="dynamic", use_container_width=True, key="mp_goals_inp")
            merged = edited.merge(grp[[_src, cty_col,"c_same","c_prev"]], on=[_src, cty_col], how="left")
            # required leads
