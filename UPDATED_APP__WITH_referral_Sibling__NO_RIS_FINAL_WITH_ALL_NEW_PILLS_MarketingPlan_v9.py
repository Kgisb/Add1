...
if view == "Marketing Plan":
    def _marketing_plan_tab():
        ...
        with left:
            mode = st.radio("Date Mode", ["MTD","Last Month","Last 90 days","Custom Range"], horizontal=True)
        with right:
            srcs = sorted(df[_src].dropna().astype(str).unique().tolist())
            sel_srcs = st.multiselect("JetLearn Deal Source", srcs, default=srcs)
        ...
        if mode == "MTD":
            ...
            c1,c2 = st.columns(2)
            with c1: A_start = _pd.Timestamp(st.date_input("Part A start", value=first_of_month))
            with c2: A_end   = _pd.Timestamp(st.date_input("Part A end", value=today))
    ...
    def _month_planning_tab():
        ...
        if plan_mode == "Leads → Enrollments":
            ...
            edited = st.data_editor(inp, num_rows="dynamic", key="mp_editor_leads", use_container_width=True, key="mp_leads_inp")
            # Compute outputs
            ...
            edited = st.data_editor(inp, num_rows="dynamic", key="mp_editor_leads", use_container_width=True, key="mp_goals_inp")
            merged = edited.merge(grp[[_src, cty_col,"c_same","c_prev"]], on=[_src, cty_col], how="left")
            # required leads
        ...
        fig, ax = plt.subplots()
        vals = [kpiA, kpiB]
        labels = ["Same-Month (A)","Preceding (B)"]
