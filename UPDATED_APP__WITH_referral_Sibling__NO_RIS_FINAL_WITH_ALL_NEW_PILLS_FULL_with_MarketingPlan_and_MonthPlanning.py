...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals", "Month Planning", "Marketing Plan","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time", "referral_Sibling", "Deal Score Trend", "Deal Score Threshold", "Invalid Deals"],
    }
...
if view == "Month Planning":
    def _month_planning_tab():
        import streamlit as st, pandas as _pd, numpy as _np, altair as alt
        ...
        _create = _pick(df, globals().get("create_col"), ["Create Date","Created Date","Deal Create Date","CreateDate","Created On"])
        _pay    = _pick(df, globals().get("pay_col"),    ["Payment Received Date","Payment Date","Enrolment Date","PaymentReceivedDate","Paid On"])
        _src    = _pick(df, globals().get("source_col"), ["JetLearn Deal Source","Deal Source","Source","_src_raw","Lead Source"])
        _cty    = _pick(df, globals().get("country_col"),["Country","Student Country","Deal Country","Lead Country"])
        _stage  = _pick(df, None, ["Deal Stage","Stage","Pipeline Stage","HS Deal Stage"])

        if not _create or not _src:
            st.warning("Please map Create Date and JetLearn Deal Source in the sidebar.", icon="⚠️"); return
        ...
        with c2:
            anchor_default = _pd.Timestamp.today().normalize().replace(day=1) - _pd.offsets.MonthBegin(1)
            anchor = st.date_input("Look-back anchor (ends at month)", value=anchor_default.date(), key="mp_anchor")
            anchor = _pd.Timestamp(anchor).replace(day=1)
        ...
        # Country handling
        cty_col = _cty if _cty and _cty in df.columns else None
        if cty_col is None:
            df["_cty2"] = "Unknown"; cty_col = "_cty2"
        ...
        # Month keys
        df["cre_m"] = df[_create].dt.to_period("M").dt.to_timestamp()
        if _pay: df["pay_m"] = df[_pay].dt.to_period("M").dt.to_timestamp()
        else: df["pay_m"] = _pd.NaT

        # Aggregate
        rows = []
        for t in months_t:
            df_t = df[df["pay_m"]==t]
            created_t = df[df["cre_m"]==t]
            created_before_t = df[df["cre_m"]<t]

            den_same = created_t.groupby([_src, cty_col]).size().rename("den_same")
            den_prev = created_before_t.groupby([_src, cty_col]).size().rename("den_prev")
            ...
            g = _pd.concat([den_same, den_prev, num_same, num_prev], axis=1).fillna(0)
            g["t"] = t
        ...
        grp = base.groupby([_src, cty_col], as_index=False).agg({"den_same":"sum","den_prev":"sum","num_same":"sum","num_prev":"sum"})
        grp["c_same"] = grp["num_same"] / grp["den_same"].replace(0, _np.nan)
        grp["c_prev"] = grp["num_prev"] / grp["den_prev"].replace(0, _np.nan)

        st.markdown("#### Conversion Baselines (Look-back)")
        st.dataframe(grp.sort_values([_src, cty_col]).reset_index(drop=True), use_container_width=True)
        st.download_button("Download Baselines CSV", data=grp.to_csv(index=False), file_name="month_planning_baselines.csv", mime="text/csv")
...
if view == "Marketing Plan":
    def _marketing_plan_tab():
        ...
        if not _create or not _src:
            st.warning("Please map Create Date and JetLearn Deal Source.", icon="⚠️"); return
        ...
        # Date modes
        def _date_modes(prefix="A"):
            today = _pd.Timestamp.today().normalize()
            first_of_month = today.replace(day=1)
            mode = st.radio(f"{prefix} Date Mode", ["MTD","Last Month","Last 90 days","Custom Range"], horizontal=True, key=f"{prefix}_date_mode")
            if mode == "MTD":
                start, end = first_of_month, today
            elif mode == "Last Month":
                end = first_of_month - _pd.offsets.Day(1); start = end.replace(day=1)
            elif mode == "Last 90 days":
                end = today; start = end - _pd.Timedelta(days=89)
            else:
                c1, c2 = st.columns(2)
                with c1: start = _pd.Timestamp(st.date_input(f"{prefix} start", value=first_of_month, key=f"{prefix}_start"))
                with c2: end = _pd.Timestamp(st.date_input(f"{prefix} end", value=today, key=f"{prefix}_end"))
            return start, end, mode

        st.markdown("### Part A (Current / Look-back Window)")
        A_start, A_end, A_mode = _date_modes("A")

        # Part B window (previous comparable period)
        today = _pd.Timestamp.today().normalize()
        if A_mode == "MTD":
            first_of_month = today.replace(day=1); prev_end = first_of_month - _pd.offsets.Day(1)
            B_start, B_end = prev_end.replace(day=1), prev_end
        elif A_mode == "Last Month":
            last_month_end = (today.replace(day=1) - _pd.offsets.Day(1)); last_month_start = last_month_end.replace(day=1)
            prior_end = last_month_start - _pd.offsets.Day(1); B_start, B_end = prior_end.replace(day=1), prior_end
        elif A_mode == "Last 90 days":
            B_end = A_start - _pd.Timedelta(days=1); B_start = B_end - _pd.Timedelta(days=89)
        else:
            delta = (A_end - A_start).days + 1; B_end = A_start - _pd.Timedelta(days=1); B_start = B_end - _pd.Timedelta(days=delta-1)
